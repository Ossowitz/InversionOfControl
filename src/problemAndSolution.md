...
В качестве первого параметра передаётся SQL-запрос. В качестве второго передаётся объект класса PersonMapper, который реализует интерфейс RowMapper.
...
*Вся информация помещается в файл database.properties. А структура файла записывается в database.properties.origin. Первый файл скрыт, 
...
Также, в конфигурационный класс - SpringConfig необходимо добавить переменную интерфейса Environment и его бин внедрить в конструктор класса. 
